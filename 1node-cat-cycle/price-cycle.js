...
async function testCat(w3, privateKey, nameFrom, addressFrom, nameTo, addressTo, fn, isAwaitReceipt) {
    ...
    try {
        ...
        let tcnt = await w3.eth.getTransactionCount(addressFrom, null);
        // if (g_bVerbose)
...
for (idxCat = g_idxCatFirst; idxCat < ( g_idxCatFirst + cntCats ); ++idxCat) {
    ...
    g_arrAllCats[idxCat].testCat = function (fn, isAwaitReceipt) {
        let self = this;
        fn = fn || function (isError, joReceipt) {};
        testCat(self.currentNode.get_w3(), self.private_key, self.name, self.address,
            self.joNextCat.name, self.joNextCat.address, fn, isAwaitReceipt);
    };
    ...
    g_arrAllCats[idxCat].run = function () {
        ...
        self.testCat(function (isError, joReceipt) {
            ++self.idxOperation;
            if (isError)
