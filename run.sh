...
_USE_DEBUG=0
_SKALED_IMAGE="skalenetwork/schain:3.5.3-ws-protocol-errors"
_DATA_DIR="./data_dir"
...
#  docker rm skale_schain_skaled
  docker run --name skale_schain_skaled -v `pwd`/${_DATA_DIR}:/data_dir ${_args_arr} -e DATA_DIR=/data_dir -e freeContractDeployment=5000 --stop-timeout 40 -i -t ${_SKALED_IMAGE} ${_options}
